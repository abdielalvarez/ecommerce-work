import React from 'react';
import candy from '../assets/static/candy.png';
import blueCar from '../assets/static/blue-car.png';
import '../assets/styles/components/Cards.scss';

function Cards (props) {
    const {productName, productPrice, counter} = props
    return (
        <section>
            <div className="card">
                <img src={candy} className="card-img-top" alt="candy" />
                <div className="part-one">
                    <h5 className="card-title text-center">{productName}</h5>
                    <h6 className="product text-center">${productPrice}</h6>
                </div>
                <div className="part-two">
                    <a href="#" className="col-xl-6 card-link">
                        <img src={blueCar} alt="car" />
                    </a>
                    <div className="col-xl-6 text-center">
                        <a href="#" className="counter hola col-xl-4">-</a>
                            <small className="counter col-xl-4">{counter}</small>
                        <a href="#" className="counter col-xl-4">+</a>     
                    </div>
                    
                </div>
            </div>
        </section>
    )
}

export default Cards;
